...
def main [] {
    ...
    if $native_result.exit_code != 0 {
        error make {msg: "The hermetic action check rejected a native Buck closure."}
    }
    ...
    for target in [
        ...
        "//tests/buck-hermetic:system_tool_path_action_alias",
    ] {
        let result = (do -i { ^nu scripts/check-buck-hermetic.nu $target } | complete)
        if $result.exit_code == 0 {
            error make {msg: $"The hermetic action check accepted fixture ($target)."}
        }
        if not ($result.stderr | str contains "contains a prohibited action or environment") {
            error make {msg: $"The hermetic action check failed without identifying fixture ($target)."}
        }
    }
}
