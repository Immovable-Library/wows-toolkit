...
def apply_filter(rows):
    ...
    only the given rows visible. Works across Excel and WPS. Adapts to the sheet
    XML namespace (prefixed x: or default) and removes any existing autoFilter
    / table parts first. The underlying data is unchanged.
    """
    if not rows:
        return
    ...
    with zipfile.ZipFile(XLSX, "r") as zin, zipfile.ZipFile(tmp, "w", zipfile.ZIP_DEFLATED) as zout:
        ...
        # namespace-aware: use the same prefix the sheet actually uses
        prefixed = "</x:sheetData>" in xml
        p = "x:" if prefixed else ""
        t = lambda tag: p + tag
        dm = re.search(r"<(?:x:)?dimension[^>]*ref=\"([^\"]+)\"", xml)
        ref = dm.group(1) if dm else "A1:AH997"

        # remove any existing autoFilter (either prefix) and table parts
        for pf in ("x:", ""):
            xml = re.sub(r"<" + pf + r"autoFilter[^>]*/>", "", xml)
            xml = re.sub(r"<" + pf + r"autoFilter[^>]*>.*?</" + pf + r"autoFilter>", "", xml, flags=re.S)
        xml = re.sub(r"<(?:x:)?tableParts[^>]*/>", "", xml)
        xml = re.sub(r"<(?:x:)?tableParts[^>]*>.*?</(?:x:)?tableParts>", "", xml, flags=re.S)

        new_auto = (
            f'<{t("autoFilter")} ref="{ref}"><{t("filterColumn")} colId="0"><{t("filters")}>'
            + "".join(f'<{t("filter")} val="{n}"/>' for n in names)
            + f'</{t("filters")}></{t("filterColumn")}></{t("autoFilter")}>'
        )
        close = f"</{t('sheetData')}>"
        xml = xml.replace(close, close + new_auto, 1)
        data = xml.encode("utf-8")

        out_entries = []
        for item in infos.values():
            if item.filename == sheet_path:
                out_entries.append((item, data))
                ...
                if item.filename.endswith(".rels") and "/worksheets/" in item.filename:
                    ...
                    d = rels.encode("utf-8")
                out_entries.append((item, d))
        for item, d in out_entries:
