...
/// Decode a camo scheme's per-stem textures into GPU-ready RGBA and classify how each reaches
/// the surface, via the shared classifier in `wowsunpack`.
pub(crate) fn build_active_camo(
    ...
    use wowsunpack::export::camo_composite::CamoApplication;
    use wowsunpack::export::camo_composite::RgbaImageData;

    // `hull_textures` is keyed by MFM path; the compositor works in stems.
    let bases: std::collections::HashMap<String, RgbaImageData> = hull_textures
        .iter()
        .map(|(path, (w, h, rgba))| {
            (mfm_stem(path).to_string(), RgbaImageData { width: *w, height: *h, pixels: rgba.clone() })
        })
        .collect();

    wowsunpack::export::camo_composite::apply_scheme(textures, uv_transforms, use_color_scheme, &bases)
        .into_iter()
        .map(|(stem, app)| {
            let active = match app {
                CamoApplication::CompositeOverBase { texture, uv } => ActiveCamo::CompositeOverBase {
                    texture: RgbaTexture { width: texture.width, height: texture.height, pixels: texture.pixels },
                    uv,
                },
                CamoApplication::Replace { texture, uv } => ActiveCamo::Replace {
                    texture: RgbaTexture { width: texture.width, height: texture.height, pixels: texture.pixels },
                    uv,
                },
            };
