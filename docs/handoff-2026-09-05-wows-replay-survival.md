...
> - **真实板厚 + build 门控**（`1e92dfaf`）：`hit_value`/`survival` 的 `zone_mm` 改走 `hull_dim::plate_thickness_for_hit`（装备 hull 的 `.geometry` 装甲网格 + `ArmorMap`，按落点射线取最近命中板，`(material_id, layer_index)`→毫米）；`HitLocation.thickness` 恒为 0 的问题随之消除（0mm/`None` 均视为未知→保守 0.33α）；命中板取「离落点最近的交点」而非「射线最先扫过的板」，避免平坦弹道掠过近侧板失真。`hit_value`/`survival` 一并门控于 `build`（`--no-default-features` 干净）。**评审后修正**：命中板改为 `|t - t_impact|` 最近者、plates 路径 0mm 过滤。
...
>
> - **SAP 溅射修复**（2026-09-20）：SAP 未穿透此前按 0 结算，与 HE 同类 bug。用 2026-09-15 AL 马可波罗局校准（SAP alpha 14,100、54 发碎弹 × α/6 = 126,900 vs 0x22 `damage_main_cs` 125,521，+1.1%；α/3 与 α/12 各偏 2 倍），`estimate_damage` 现对 HE/SAP 未穿透统一按 α/6、`pen_verdict`/`ribbon_for`/lesson 文案标「HE溅射 / SAP溅射」，AP+SAP 船不再收到「换 HE」建议。
>
> **未结项**：该局 SAP 零穿透（54 碎弹 + 1 跳弹），所以 **SAP 穿透按 α/3 未验证**；SAP 跳弹是否结算溅射也缺样本（现按 AP 语义记 0）。下次扫到含 SAP 舰体穿透（目标存活、非过杀）的对局，用 `damage_main_cs`/`hits_main_cs` 复核。识别 SAP 以数据为准（`replayshark events` 看 `shell.ammo_type`），不按国籍推断。另有两项改动前遗留、待排期：① `volley_verdict` 与深层命中的地形命中（`HIT_GROUND`）未过滤，会让 AP 估伤偏高（2026-09-15 局 +15.9%）；② survival 侧的 incoming 同样计入地形命中。
>
...
5. **per-ship 缓存**（评审 MEDIUM；本会话评估后回退）：`hull_data_for_report` 每回放重读 assets.bin + VFS + 全量 `paths_storage` 扫描，跨回放无记忆化。曾试改 `hull_data_for_report` 加 `&mut HullDataCache`（按 `(version, model_path)` 缓存 `HullData`），但仅为每模型几何/擦洗缓存、仍每回放重读 assets.bin，且需穿线 4 个近似相同的 replayshark 循环，风险/收益不划算——已回退（工作树干净）。批处理优化应：按 (version, source) hoist 一次 VFS + parsed `PrototypeDatabase`（自引用借用，需 `self_cell`/arena 或 Box::leak），缓存 `HullData` 按模型目录。
...
7. **~~半开/闭区间~~（已修，本会话）**：`exact_zone_for_hit` 已镜像游戏 `getSplashBoxNameAtPoint` 的半开 `[min, max)`（`<` 而非 `<=`）；落在 `max` 面上的点归相邻框，浮点精度下重叠时取首个包含框（已注明）。
8. **~~`build_zones` last-wins 非确定~~（已修，本会话）**：`build_zones` 改为「框名只被一个键认领→映射；被 ≥2 个键认领→丢弃（落回粗 label）」，结果与 `HashMap` 迭代顺序无关（无需排序即确定）；新增单测覆盖「同名框被两键认领则丢弃」。
