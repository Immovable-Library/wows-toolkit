...
> 继续 WOWS 回放生存端评估开发。先读 `docs/handoff-2026-09-05-wows-replay-survival.md` 与 `C:/Users/asdfg/.codex/skills/wows-replay-parser/specs/2026-09-05-survival-evaluation-engine.md`。**S1-S4 生存端评估引擎已全部实现且 S1 打分已诚实化**：`replayshark` 新增 `survival` 子命令（默认 JSONL / `--text` 文本）；`wows-replay-insights/src/survival.rs` 产出 `SurvivalProfile`（含 `Exposure`/`HpTimeline`/`OutputCoupling`）；复用 `hit_value::estimate_damage` 与「仅敌方」逻辑；`BattleReport::positions_over_time()` / `hp_timeline()` 已暴露。**S1 诚实化**：`died`/`survival`/`avoidance_ratio`/`self_rescue`/`taken_frac_of_potential`/`survival_score`/`grade` 改为 `Option`；规避比按炮弹 lane 对齐、存在未识别命中/非炮弹伤害时置 `null`；复合分封顶在"已测量权重占比"；新增 `match_complete`/`shell_potential_damage`/`non_shell_potential_damage`；`has_dcp` 三态；去掉 `u32::MAX` 哨兵。**已修 bug**：`victim_entity_id` 改 `Option<EntityId>` 全链贯通；死船过期 `Transform3d` 排除；unidentified 三分；S2 approach/kiting 距离化+插值+自舰位移门槛。**S2**：`PositionHistoryLog`+近似暴露/走位。**S3**：`HealthHistoryLog`(按变化去重)+`HpTimeline`(最低/结束血 frac、濒死<20%、最低血时刻、累计修复回血>1、血线以下时间占比含头尾跨度)。**S4**：`OutputCoupling`——自舰打敌命中按血量分带，输出估伤/DPM(满血 vs 低血)/低血输出占比 + 价值中立耦合结论(非因果)；补未估命中计数、饱和/口径披露。**下一步**：可选整合评分(把 S1 生存分+输出端 hit_value 报告并成"整局表现")，或 H1/M4 解锁(visibilityFlags 逐目标被点亮)。`games_dir=D:/World_of_Warships`。
