...
> **精确对账（P3b，本会话）**：`HullData`（dims + `.splash` SplashBox zones）+ `exact_zone_for_hit`（体偏移→模型坐标点-框分类→精确 `HitLocation` 键）；`assess`/`self_output_timeline`/`survival::assess` 用精确键取厚度/饱和，粗粒度 label 保留给 reason/报告 zone。真跑 Georgia Atoll 饱和命中 7→13（精确区 `max_hp` 池生效）。评审已修：`exact_zone_for_hit` 首框未映射时继续扫描（引擎框嵌 cit 框）；饱和记账与 citadel 门改按精确键。已知：当前安装 `HitLocation.thickness` 为 0（HE 全穿语义，非 bug）；板厚/区键仍可能来自 stock hull；半开/闭区间、`exact_zone` 报告字段、同名框 last-wins 为后续项。
> **整局表现（P4，本会话）**：`performance` 命令（`replayshark performance [--text]`）：`performance.rs` `WholeMatch{survival, output}` + `assess_whole` + `render_whole` 一页文本；`OutputSummary` 主炮估伤/命中/未评估、轮均分/打中轮数/最高、饱和命中、逐目标 lessons。默认 JSON，`--text` 一页。真跑 Georgia Atoll 出结果。口径：输出端主炮估伤 383591 与生存 S4 更宽 412218 已用「主炮」/「输出估伤」区分。
> **剩余可选项**：`exact_zone` 审计字段（报告暴露精确区键，审计饱和/厚度）；每舰缓存（`hull_data_for_report` 跨回放缓存 assets.bin/VFS 解析，批处理优化）；per-hull `hit_locations` 取板厚；H1/M4 解 `visibilityFlags` 逐目标被点亮（标"需数据"，暂缓）。
