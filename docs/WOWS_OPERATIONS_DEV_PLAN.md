...
击杀数（frags）单独不产生显著经验加成：局内去均值回归中，控制 `ship_eff` + 点亮后 frags 系数仅 +0.012（每击杀约 +1.2%），池子层面 `team_frags` 系数约 -0.0013。击杀的价值已包含在吃船比例里，补刀本身几乎不额外奖励。见 `scripts/analyze_frags.py`。
...
- 挂机与暴毙是两种行为，但 replay 公开数据里没有队友的 `is_afk`/`is_ineffective` 标志（只在记录者自己的 privateDataList），只能行为近似：零伤害+零击杀+零点亮≈挂机（池子 x0.927，样本仅 5 局），有少量输出但早死≈暴毙（x0.970）。玩家反馈的“挂机扣 1/7”在本样本未证实（1/7=-14.3%），需服务器判定样本进一步验证。见 `scripts/analyze_afk_death.py`。
